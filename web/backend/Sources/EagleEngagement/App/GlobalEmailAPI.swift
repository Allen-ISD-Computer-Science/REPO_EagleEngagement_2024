...
/*        guard !Validator.allenEmail.validate(emailData.contact.emailAddress).isFailure else {
        ...
        guard let notificationAPI = Environment.get("NOTIFICATION_API_URI") else {
            fatalError("Failed to determine NOTIFICATION_API_URI from environment");
        }

        let notificationAPIURI = URI(string: notificationAPI);
        
        guard let notificationAPIKey = Environment.get("NOTIFICATION_API_KEY") else {
            fatalError("Failed to determine NOTIFICATION_API_KEY from environment");
        }

        let response: ClientResponse = try await request.client.post(notificationAPIURI) { req in
            try req.content.encode(emailData, as: .json)

            req.headers.add(name: "apiKey", value: notificationAPIKey)
        }
